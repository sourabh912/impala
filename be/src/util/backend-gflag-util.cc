...
DECLARE_bool(enable_catalogd_cache_sync_to_latest_event_id);
...
Status PopulateThriftBackendGflags(TBackendGflags& cfg) {
  ...
  cfg.__set_enable_catalogd_cache_sync_to_latest_event_id(
        FLAGS_enable_catalogd_cache_sync_to_latest_event_id);
  return Status::OK();
}
