    ...
    try {
      ...
      if (events.size() == 0) {
        if (ignoreUnknownDb) {
          ...
          throw new MetaException(
              String.format("Drop database event not received for db: %s from event id: %s. "
                  + "Check if notification events are configured in hive metastore",
                  dbName, beforeDropEventId));
        }
      }
